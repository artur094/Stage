    ...
    for category in Category.objects.all().filter(rsa=me):
        ...
        if category.name == "relatives' post":
            for relative in Relative.objects.all().filter(rsa=me):
                return HttpResponse(instagram.search_user(relative.username, token)+' '+token)
...
def settings(request):
    ...
    relatives = Relative.objects.all().filter(rsa=me)

    return render(request, 'settings.html', { 'user':me, 'categories': my_categories,'relatives_name':"relatives' post",'relatives':relatives})
